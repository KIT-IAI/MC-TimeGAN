...
class GridManager:
    ...
    def create_grid(
        ...
        bus_hv, bus_lv1, bus_lv2 = (
            pp.create_bus(grid, hv),
        ...
        for _ in range(1, max(len(value) for value in inputfeed.values())):
            ...
            if mode == "row":
                ...
                pp.create_line(
                    grid, bus_lv1, grid["bus"].index[-1], l_lv, std_type="NAYY 4x150 SE"
                )
        ...
        if mode == "default":
            for key, items in inputfeed.items():
                for i, _ in enumerate(items):
                    if key == "load":
                        pp.create_load(
                            grid, len(grid["bus"]) - 1, p_mw=1, name="load_" + str(i)
                        )
                    elif key == "sgen":
                        pp.create_sgen(
                            grid, len(grid["bus"]) - 1, p_mw=1, name="sgen_" + str(i)
                        )
        else:
            for key, items in inputfeed.items():
                for i, _ in enumerate(items):
    ...
    def correct_bus_names(
        self, grid: pp.auxiliary.pandapowerNet
    ) -> pp.auxiliary.pandapowerNet:
    ...
    def plot_annotated_grid(self, grid: pp.auxiliary.pandapowerNet) -> None:
        ...
        for i, x, y, _ in grid.bus_geodata.itertuples():
            ax.annotate(
                (
                    " " + str(i)
                    if grid.bus["name"][i] is None
                    else " " + grid.bus["name"][i][grid.bus["name"][i].rfind(" ") + 1 :]
                ),
                (x, y),
            )
        plt.show()
    ...
    def datasource(self, inputfeed: dict) -> tuple:
        ...
        for _, items in inputfeed.items():
            for profile, name in items:
                new_df = pd.DataFrame(
                    profile.to_numpy(), index=profile.index, columns=[name]
                )
    ...
    def time_series(
        self, inputfeed: dict, output_dir: str, grid: pp.auxiliary.pandapowerNet
    ) -> None:
    ...
    def extract_feeder(
        self, input_grid: pp.auxiliary.pandapowerNet, busses: list, keys: list
    ) -> pp.auxiliary.pandapowerNet:
        ...
        for key in keys:
            if key == "bus":
                ...
                for bool_list in [bus != grid[key]["bus"] for bus in busses]:
                    grid[key]["bus"] = grid[key][["bus"]].where(
                        bool_list, np.nan, axis=0
                    )
    ...
    def load_data_from_directory(
        self, directory: str, data_type: str, file_name: str
    ) -> pd.DataFrame:
        """

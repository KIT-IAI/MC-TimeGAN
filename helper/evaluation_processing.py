...
class Evaluation:
    ...
    def count_datapoints(self, df: pd.DataFrame, threshold: float) -> int:
        ...
        print(
            pd.DataFrame(temp_dict, index=["No Overshoot", "Overshoot"]).to_markdown()
        )
    ...
    def prepare_data(
        ...
        *inputs: tuple,
        ...
        for data, bool_scale in inputs:
            ...
            data_stack = np.stack(
                [data[i : i + horizon] for i in range(len(data) - horizon)]
            )
            if shuffle_stack:
                data_stack = shuffle(data_stack, random_state=random_state)
            (
                return_list.extend([data_stack, scaler.data_max_, scaler.data_min_])
                if bool_scale
                else return_list.extend([data_stack])
            )
        return return_list if len(return_list) > 1 else return_list.pop()

    def low_dimensional_representation(
        self,
        ...
        data_original, data_synthetic = (
            np.asarray(data_original)[idx],
            np.asarray(data_synthetic)[idx],
        )
        no, seq_len, dim = data_original.shape
        prep_data = np.array(
            [
                np.reshape(np.mean(data_original[i, :, :], 1), [1, seq_len])
                .flatten()
                .tolist()
        ...
        prep_data_hat = np.array(
            [
                np.reshape(np.mean(data_synthetic[i, :, :], 1), [1, seq_len])
                .flatten()
                .tolist()
                for i in range(sample_no)
            ]
    ...
    def distribution_estimate(
        self, data_original: np.ndarray, data_synthetic: np.ndarray, technique: str
    ) -> None:
        assert technique in ["pca", "tsne"]
        sample_no = min([1000, len(data_original)])
        np.random.seed(42)
        ...
        if technique == "pca":
            ...
            fig, ax = plt.subplots()
            ax.scatter(
                tsne_results[:sample_no, 0],
                tsne_results[:sample_no, 1],
                c=colors[:sample_no],
                alpha=0.2,
                label="Original",
            )
    ...
    def numeric_evaluation(self, do: pd.DataFrame, ds: pd.DataFrame) -> None:
        ...
        print(
            pd.DataFrame(
                temp_list, index=index, columns=do.columns.to_list()
        ...
        for (_, do), (_, ds) in zip(df_o.items(), df_s.items()):
            ...
            if not len(limits) > 1:
                ax.set_ylim(
                    0,
                    (
                        1.05 * max(do.max(), ds.max())
                        if max(do.max(), ds.max()) > limits[0]
                        else 1.05 * limits[0]
                    ),
                )
            else:
                ax.set_ylim(
                    (
                        0.995 * min(do.min(), ds.min())
                        if min(do.min(), ds.min()) < limits[-1]
                        else 0.995 * limits[-1]
                    ),
                    (
                        1.005 * max(do.max(), ds.max())
                        if max(do.max(), ds.max()) > limits[0]
                        else 1.005 * limits[0]
                    ),
                )
    ...
    def plot_cumsum(self, *dfs: pd.DataFrame, yname: str) -> None:
        ...
        indices = (
            np.random.choice(dfs[0].shape[-1], len(colors))
            if dfs[0].shape[-1] > len(colors)
            else range(dfs[0].shape[-1])
        )
        fig, ax = plt.subplots(figsize=(15, 4))
        for i, df in enumerate(dfs):
            for j, index in enumerate(indices):
                ax.plot(
                    df.iloc[:, [index]].cumsum(),
                    color=colors[j],
                    ls="-" if i == 0 else "--",
                    marker="" if i == 0 else markers[j],
                    markevery=0.05,
                    label=df.columns[j],
                )
        ax.legend()
    ...
    def plot_acf_and_pacf(
        self, *dfs: pd.DataFrame, acf_lags: int = None, pacf_lags: int = None
    ) -> None:
        fig, ax = plt.subplots(
            dfs[0].shape[-1],
            2,
            figsize=(12, 3 * dfs[0].shape[-1]),
            layout="constrained",
        )
        for i, df in enumerate(dfs):
            for j, ((_, data)) in enumerate(df.items()):
                ...
                ax[j, 0].plot(
                    range(len(data)) if acf_lags is None else range(acf_lags + 1),
                    acf(data, nlags=len(data) if acf_lags is None else acf_lags),
                    alpha=1 if i == 0 else 0.75,
                )
                ax[j, 1].stem(
                    range(96 + 1) if pacf_lags is None else range(pacf_lags + 1),
                    pacf(
                        data, nlags=96 if pacf_lags is None else pacf_lags, method="ols"
                    ),
                    linefmt="C0-" if i == 0 else "C1-",
                    markerfmt="o" if i == 0 else "x",
                )
                ax[j, 0].set_xlim(
                    -0.1, len(data) + 0.1 if acf_lags is None else acf_lags + 0.1
                )
                ax[j, 1].set_xlim(
                    -0.1, 96 + 0.1 if pacf_lags is None else pacf_lags + 0.1
                )
    ...
    def prepare_violin(
        self, *dfs: pd.DataFrame, names: list = ["Data", "Profile", "Type"]
    ) -> pd.DataFrame:
    ...
    def plot_violin(
        self, res_line_original: pd.DataFrame, res_line_synthetic: pd.DataFrame
    ) -> None:
    ...
    def plot_profile_violin_and_cumsum(
        self, load_original: pd.DataFrame, load_synthetic: pd.DataFrame
    ) -> None:
        fig, ax = plt.subplots(
            load_original.shape[1], 2, figsize=(15, 4 * load_original.shape[1])
        )
        for i, _ in enumerate(load_original.items()):
            df_violin = self.prepare_violin(
                load_original.iloc[:, [i]], load_synthetic.iloc[:, [i]]
            )
            sns.violinplot(
                data=df_violin,
                x="Data",
    ...
    def plot_acf_comparison(
        ...
        fig, ax = plt.subplots(
            load_original.shape[1],
            2,
            figsize=(12, 3 * load_original.shape[1]),
            layout="constrained",
        )
        for i, ((_, do), (_, ds)) in enumerate(
            zip(load_original.items(), load_synthetic.items())
        ):
    ...
    def plot_psd(
        self,
        load_original: pd.DataFrame,
        load_synthetic: pd.DataFrame,
        column_index: int = 4,
        fs: float = 1e-5,
        nperseg: int = 1024,
    ) -> None:
        f_synthetic, psd_synthetic = signal.welch(
            load_synthetic.to_numpy()[:, column_index], fs=fs, nperseg=nperseg
        )
        f_original, psd_original = signal.welch(
            load_original.to_numpy()[:, column_index], fs=fs, nperseg=nperseg
        )
        fig, ax = plt.subplots(figsize=(15, 4))
    ...
    def plot_trafo_line_voltage(
        self, trafo, line, voltage, saving=(False, ""), fontsize=12
    ):
        ...
        for i, grid_state_metric in enumerate([trafo, line, voltage]):
            ax[i].plot(
                grid_state_metric[0], label="Original", linewidth=1.2
            )  # Use separate marker and linestyle
            ax[i].plot(
                grid_state_metric[-1], label="MC-TimeGAN", linewidth=1.2
            )  # Use separate marker and linestyle
            if i == 0:
            ...
            ylabel = [r"Transformer Load (\%)", r"Line Load (\%)", r"Bus Voltage (pu)"][
                i
            ]
            ax[i].set_ylabel(ylabel, fontsize=fontsize)
            ax[i].set_xticks(np.arange(0, len(grid_state_metric[0]) + 1, 96))
            ax[i].set_xticklabels(
                np.arange(0, (len(grid_state_metric[0]) + 1) / 96, 1, dtype=int),
                fontsize=fontsize,
            )
            ax[i].grid(True)
        ...
        if saving[0]:
            pgf_folder = os.path.join(
                "helper", "output", "single_feeder_grid", "figures"
            )

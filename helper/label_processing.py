...
class LabelProcessor:
    ...
    def plot_profiles_and_labels(
        self, df_profiles: pd.DataFrame, df_labels: pd.DataFrame
    ) -> None:
        """Visualize profile and corresponding labels in a joint plot."""
        assert (
            df_profiles.shape == df_labels.shape
        ), f"Profiles shape: {df_profiles.shape}, Labels shape: {df_labels.shape}"
    ...
    def binary_labels(self, df: pd.DataFrame, threshold: float = None) -> pd.DataFrame:
        ...
        for column, data in df.items():
            ...
            labels = (
                zero_series.mask(data > threshold * data.max(), 1)
                if threshold is not None
                else data.mask(data != 0, 1).astype(int)
            )
    ...
    def discretize(
        self, df: pd.DataFrame, bin_by: str, factor: int = 1, mean_offset: bool = True
    ) -> pd.DataFrame:
        ...
        for column, data in df.items():
            ...
            if mean_offset:
                bins = [
                    data.mean() + factor * bin_value * i
                    for i in range(
                        -int(
                            np.ceil((data.mean() - data.min()) / (factor * bin_value))
                        ),
                        int(np.ceil((data.max() - data.mean()) / (factor * bin_value)))
                        + 1,
                    )
                ]
            else:
                bins = [
                    factor * bin_value * i
                    for i in range(
                        int(np.ceil((data.max() - data.min()) / (factor * bin_value)))
                        + 2
                    )
                ]
        ...
        return pd.DataFrame(
            {
                "season": [
                    month
                    for month in range(months[0], months[-1])
                    for _ in range(no_dp)
                ]
            }
        )

    def season_magnitude_labels(
        self, months: tuple, no_dp: int = 96 * 7 * 4
    ) -> pd.DataFrame:
        ...
        return pd.DataFrame(
            {
                "season": [
                    list(chain(range(1, 6), range(6, -1, -1)))[i]
    ...
    def peak_spreading_approach(
        self, df: pd.DataFrame, no_neighbors: int, target_label_list: list = None
    ) -> pd.DataFrame:
        ...
        for i, (column, data) in enumerate(df.items()):
            ...
            target_label = (
                ...
                else target_label_list[i]
            )

            data_ = data.copy()
            for window in data.eq(target_label).rolling(
                no_neighbors, min_periods=1, closed="both"
            ):
                if window.any():
            ...
            print(
                pd.DataFrame(
                    [hist, hist_], index=["Before", "After"], columns=ordinal_labels
                )
                .rename_axis(column)
                .to_markdown(),
                "\n",
    ...
    def peak_elevation_approach(
        self, df: pd.DataFrame, no_neighbors: int, threshold: int
    ) -> pd.DataFrame:
        ...
        for column, data in df.items():
            ...
            for i, window in enumerate(
                data.rolling(
                    2 * no_neighbors + 1, min_periods=1, center=True, closed="both"
                )
            ):
            ...
            neighbor_label_means = {
                unique_label: labels_stats["mean"]
                .where(labels_stats["label"].eq(unique_label))
                .mean()
                for unique_label in ordinal_labels
            }
            labels_stats = pd.concat(
                [
                    labels_stats,
                    pd.Series(
                        [
                            neighbor_label_means[label]
                            for label in labels_stats["label"]
                        ],
                        name="label_mean",
                    ),
                ],
            ...
            for _, label, mean, label_mean in labels_stats.itertuples():
                if mean / label_mean > threshold:
                    result_list.extend(
                        [label + 1] if label != max(ordinal_labels) else [label]
                    )
                else:
            ...
            print(
                pd.DataFrame(
                    ...
                    columns=(
                        ordinal_labels
                        if len(ordinal_labels) > len(ordinal_labels_)
                        else ordinal_labels_
                    ),
                )
                .rename_axis(column)
                .to_markdown(),
...
class TimeSeriesSimulator:
    def __init__(self, sb_code, days):
        ...
        self.time_steps = range(
            96 * days
        )  # 15-minute intervals for the specified number of days
        self.label_processor = LabelProcessor()

    def load_net(self, sb_code):
        """Load the SimBench network based on the provided code."""
        net = sb.get_simbench_net(sb_code)
        assert not sb.profiles_are_missing(
            net
        ), "Profiles are missing in the loaded network."
        return net
    ...
    def run_power_flow(self, profiles):
        ...
        results = pd.DataFrame(
            [], index=self.time_steps, columns=["Load Sum", "min_vm_pu", "max_vm_pu"]
        )
    ...
    def plot_profiles(self, results, profiles):
        ...
        load_profile_names = list(
            load_profile_names.loc[
                load_profile_names.str.contains("H0")
                & load_profile_names.str.contains("pload")
            ]
        )
        load_profiles = self.net.profiles["load"].loc[
            self.time_steps, load_profile_names
        ]
        ...
        fig, ax = plt.subplots()
        ax.plot(
            results.loc[:, ["min_vm_pu", "max_vm_pu"]], label=["min_vm_pu", "max_vm_pu"]
        )
    ...
    def collect_data_at_pv_buses(self, profiles):
        ...
        for series in [bus == self.net.load["bus"] for bus in self.net.sgen["bus"]]:
            ...
            bus_id_list.extend(
                list(self.net.load.loc[list(series.loc[series.tolist()].index), "bus"])
            )

        sgen_active_power_data = profiles[("sgen", "p_mw")].set_axis(
            [
                f"{name[name.find(' ') + 1:]} @Bus {bus_id}"
                for name, bus_id in zip(
                    self.net.sgen["name"].tolist(), self.net.sgen["bus"].tolist()
                )
            ],
            axis=1,
        )
        ...
        load_active_power_data = (
            ...
            .set_axis(
                [
                    f"{name[name.find(' ') + 1:]} @Bus {bus_id}"
                    for name, bus_id in zip(
                        self.net.load["name"].tolist()[:valid_columns],
                        bus_id_list[:valid_columns],
                    )
                ],
                axis=1,
            )
    ...
    def run_analysis(self):
        ...
        self.sgen_active_power_data, self.load_active_power_data = (
            self.collect_data_at_pv_buses(self.profiles)
        )

        self.plot_profile(
            self.sgen_active_power_data.loc[self.time_steps], "PV profiles"
        )
        self.plot_profile(
            self.load_active_power_data.loc[self.time_steps],
            "Load profiles at buses with PV",
        )
        self.plot_profile(self.sgen_active_power_data, "PV profiles (whole year)")
        self.plot_profile(
            self.load_active_power_data, "Load profiles at buses with PV (whole year)"
        )

    def run_label_prepare_process(self):
        """Run the label preparation process."""
        print("Binary labels for load active power")
        binary_labels_load = self.label_processor.binary_labels(
            self.load_active_power_data.iloc[self.time_steps, [0]], 0.7
        )
        self.label_processor.plot_profiles_and_labels(
            self.load_active_power_data.iloc[self.time_steps, [0]], binary_labels_load
        )

        print("Binary labels for sgen active power")
        binary_labels_sgen = self.label_processor.binary_labels(
            self.sgen_active_power_data.iloc[self.time_steps, [0]]
        )
        self.label_processor.plot_profiles_and_labels(
            self.sgen_active_power_data.iloc[self.time_steps, [0]], binary_labels_sgen
        )

        print("Three sigma labels for load active power")
        three_sigma_labels = self.label_processor.three_sigma_labels(
            self.load_active_power_data.iloc[self.time_steps, [0]]
        )
        self.label_processor.plot_profiles_and_labels(
            self.load_active_power_data.iloc[self.time_steps, [0]], three_sigma_labels
        )

        print("Five sigma labels for load active power")
        five_sigma_labels = self.label_processor.five_sigma_labels(
            self.load_active_power_data.iloc[self.time_steps, [0]]
        )
        self.label_processor.plot_profiles_and_labels(
            self.load_active_power_data.iloc[self.time_steps, [0]], five_sigma_labels
        )

        print("Discretized labels for load active power")
        discretized_labels_load = self.label_processor.discretize(
            self.load_active_power_data.iloc[self.time_steps, [0]], bin_by="std"
        )
        self.label_processor.plot_profiles_and_labels(
            self.load_active_power_data.iloc[self.time_steps, [0]],
            discretized_labels_load,
        )
        print("Discretized labels for sgen active power")
        discretized_labels_sgen = self.label_processor.discretize(
            self.sgen_active_power_data.iloc[self.time_steps, [0]], bin_by="std"
        )
        self.label_processor.plot_profiles_and_labels(
            self.sgen_active_power_data.iloc[self.time_steps, [0]],
            discretized_labels_sgen,
        )
        print("Season labels for sgen active power (Months)")
        season_labels = self.label_processor.season_labels((0, 4))
        self.label_processor.plot_profiles_and_labels(
            self.sgen_active_power_data.iloc[: 4 * (96 * 7 * 4), [0]], season_labels
        )
        print("Season magnitude labels for sgen active power")
        season_magnitude_labels = self.label_processor.season_magnitude_labels(
            (0, 12), int(self.sgen_active_power_data.shape[0] / 12)
        )

        # Ensure season_magnitude_labels has the correct shape
        season_magnitude_labels = season_magnitude_labels.iloc[
            : self.sgen_active_power_data.shape[0]
        ]
        season_magnitude_labels.columns = ["season"]

        self.label_processor.plot_profiles_and_labels(
            self.sgen_active_power_data.iloc[:, [0]], season_magnitude_labels
        )
    ...
    def run_peak_spread_label(self):
        ...
        peak_spread_labels = self.label_processor.peak_spreading_approach(
            df=five_sigma_labels, no_neighbors=2, target_label_list=None
        )
        self.label_processor.plot_profiles_and_labels(
            self.load_active_power_data.iloc[self.time_steps, [0]], peak_spread_labels
        )
    ...
    def run_elevated_label(self):
        ...
        peak_elevated_labels = self.label_processor.peak_elevation_approach(
            df=five_sigma_labels, no_neighbors=20, threshold=1.25
        )
        self.label_processor.plot_profiles_and_labels(
            self.load_active_power_data.iloc[self.time_steps, [0]], peak_elevated_labels
        )
